...
import { applyFixtureEvidenceCommand, FixtureCommandError } from "@/lib/development-fixtures/commands";
import { attemptMutationResponseSchema, attemptResponseSchema } from "@/lib/lesson/runtime-contracts";
...
const schema = z.object({
  ...
  responseLatencyMs: z.number().int().nonnegative().max(120_000),
}).strict();
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  if (access.fixture) {
    try {
      return NextResponse.json(await applyFixtureEvidenceCommand(attemptId, parsed.data));
    } catch (error) {
      return NextResponse.json(
        { error: error instanceof FixtureCommandError ? error.code : "fixture_evidence_not_saved" },
  ...
  const result = await supabase.rpc("command_child_attempt", {
    ...
    p_command: "submit_response",
    p_payload: { response: parsed.data.response, responseLatencyMs: parsed.data.responseLatencyMs },
  });
  ...
  if (result.error || !response.success) {
    ...
    return NextResponse.json({ error: stale ? "stale_attempt_state" : "evidence_not_saved" }, { status: stale ? 409 : 403 });
  }
