...
export function ChildShell({ children, fixture = false }: { children: ReactNode; fixture?: boolean }) {
  return <div className={`${styles.childShell} ${fixture ? styles.fixtureActive : ""}`}>{fixture && <div className={styles.fixtureBanner}>FIXTURE DATA · LOCAL ONLY · NOT SAVED TO SUPABASE</div>}<header className={styles.childBar}><div className={styles.brandLink}><AcademyMark compact /><span><strong>Alonso Academy</strong><small>My learning space</small></span></div><div className={styles.childIdentity}><span className={styles.childName}>Alonso</span><form action={signOut}><button type="submit">Sign out</button></form></div></header>{children}</div>;
}
